...
- Features: **Pitch F₀** + **Harmonicitet** (autocorrelation) + kroma
- Chroma → Krumhansl tonartsestimat (visas i score-raden)
- **Nashville-remsa** under tidslinjen (toggle **Nashville**): stora siffror över tid från kroma-topp i glidande fönster (ackordlikt); F₀ som fallback
- Små Nashville-badges på objekt när F₀ finns i objektfönstret
- Sparas i `.wosscore.json` / CSV (`Pitch_F0`, `Harmonicity`)
- Låg tonartskonfidens → ingen remsa
...
`ScoreObject.rmsEnergy`, `ScoreObject.nashville`, `ScoreObject.pitchClass`.
