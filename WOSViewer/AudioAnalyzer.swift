...
struct AudioAnalyzer {
    ...
    func analyzeAudio(at url: URL) throws -> AnalysisResult {
        ...
        return AnalysisResult(
            ...
            duration: Double(mono.samples.count) / mono.sampleRate,
            hopLength: hopLength,
    ...
    private func writeCSV(
        ...
        var lines = ["Time_Seconds,RMS_Energy,Spectral_Centroid,ZCR,Novelty_Curve"]
        for i in 0..<times.count {
            lines.append(String(
                format: "%.6f,%.8f,%.6f,%.8f,%.8f",
                times[i], rms[i], centroid[i], zcr[i], novelty[i]
            ))
        }
